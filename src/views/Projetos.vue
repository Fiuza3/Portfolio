    ...
    <div class="container mx-auto px-6">
      <div class="text-center mb-16">
        ...
        <p class="text-xl text-gray-600 max-w-3xl mx-auto">
          Uma coleção dos meus trabalhos mais significativos, demonstrando minhas 
      ...
      <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        >
          ...
          <div class="relative h-48 bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center">
            <span class="text-white text-lg font-semibold">{{ projeto.titulo }}</span>
          ...
          <!-- Conteúdo do Card -->
          <div class="p-6">
            <h3 class="text-xl font-bold text-gray-900 mb-3">{{ projeto.titulo }}</h3>
            <p class="text-gray-600 mb-4 line-clamp-3">{{ projeto.descricao }}</p>
      ...
      <div v-if="projetosFiltrados.length === 0" class="text-center py-12">
        <p class="text-gray-500 text-lg">Nenhum projeto encontrado para este filtro.</p>
      </div>

      <!-- Call to Action -->
      <div class="text-center mt-16">
        <div class="bg-primary-50 rounded-2xl p-8">
          <h3 class="text-2xl font-bold text-gray-900 mb-4">Gostou do que viu?</h3>
      ...
      >
        <div class="p-6">
          <div class="flex justify-between items-start mb-4">
            <h3 class="text-2xl font-bold text-gray-900">{{ projetoSelecionado?.titulo }}</h3>
            <button 
              @click="fecharModal"
          ...
          <div class="h-48 bg-gradient-to-br from-primary-400 to-primary-600 rounded-lg mb-6 flex items-center justify-center">
            <span class="text-white text-xl font-semibold">{{ projetoSelecionado?.titulo }}</span>
          </div>
          
          <p class="text-gray-600 mb-6">{{ projetoSelecionado?.descricao }}</p>
          
          <div class="mb-6">
            <h4 class="font-semibold text-gray-900 mb-3">Tecnologias Utilizadas:</h4>
          ...
          <div class="flex gap-4">
            <a 
              v-if="projetoSelecionado?.linkDemo" 
              :href="projetoSelecionado.linkDemo" 
              target="_blank"
...
const projetosFiltrados = computed(() => {
  ...
  switch (filtroAtivo.value) {
    ...
    case 'frontend':
      return projetos.filter(p => 
        p.tecnologias.some(tech => 
          ['Vue', 'React', 'Angular', 'TypeScript'].some(frontend => 
    ...
    case 'fullstack':
      return projetos.filter(p => 
        p.tecnologias.some(tech => 
          ['Node.js', 'Express', 'MongoDB', 'PostgreSQL'].some(backend => 
